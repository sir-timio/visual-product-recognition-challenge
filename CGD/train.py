...
from utils import recall, LabelSmoothingCrossEntropyLoss, BatchHardTripletLoss, ImageReader, MPerClassSampler, Product10KDataset
...
def test(net, recall_ids):
    ...
    with torch.no_grad():
        ...
        total_loss, total_num, total_correct = 0, 0, 0
        ...
        if data_name == 'isc':
            ...
            acc_list = [0 for _ in range(len(recall_ids))]
            # acc_list = recall(eval_dict['test']['features'], test_data_set.labels, recall_ids)
    ...
    # print(desc)
    # return total_loss / total_correct, acc_list[0]
    return total_loss / total_correct, total_correct / total_num
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_name', default='car', type=str, choices=['car', 'cub', 'sop', 'isc', 'products-10k'],
                        help='dataset name')
    ...
    if data_name == 'products-10k':
        train_data_set = Product10KDataset(f'{data_path}/{data_name}', annotation_file='train.csv')
    else:
        train_data_set = ImageReader(data_path, data_name, 'train', crop_type)
    ...
    # test_data_set = ImageReader(data_path, data_name, 'query' if data_name == 'isc' else 'test', crop_type)        train_data_set = Product10KDataset(data_path, annotation_file='train.csv')
    if data_name == 'products-10k':
        test_data_set = Product10KDataset(f'{data_path}/{data_name}', annotation_file='test.csv')
    else:
        test_data_set = ImageReader(data_path, data_name, 'query' if data_name == 'isc' else 'test', crop_type)
    test_data_loader = DataLoader(test_data_set, batch_size, shuffle=False, num_workers=6)
    eval_dict = {'test': {'data_loader': test_data_loader}}
    if data_name == 'isc':
        gallery_data_set = ImageReader(data_path, data_name, 'gallery', crop_type)
        gallery_data_loader = DataLoader(gallery_data_set, batch_size, shuffle=False, num_workers=6)
        eval_dict['gallery'] = {'data_loader': gallery_data_loader}

    # model setup, model profile, optimizer config and loss definition
    model = Model(backbone_type, gd_config, feature_dim, num_classes=train_data_set.num_classes).cuda()
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        test_loss, test_accuracy = test(model, recalls)
        lr_scheduler.step()
        ...
        if test_loss < best_loss:
            torch.save(model.state_dict(), 'results/{}_model_best.pth'.format(save_name_pre))
            best_loss = test_loss
        if epoch % 10 == 0:
