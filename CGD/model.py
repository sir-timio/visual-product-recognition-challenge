...
class Model(nn.Module):
    def __init__(self, backbone_type, gd_config, feature_dim, num_classes):
        ...
        if backbone_type == 'resnet50':
            ...
            aux_features = backbone._conv_head.out_channels
        ...
        for i in range(n):
            ...
            self.main_modules.append(nn.Sequential(nn.Linear(aux_features, k, bias=False), L2Norm()))
        self.global_descriptors = nn.ModuleList(self.global_descriptors)
        self.main_modules = nn.ModuleList(self.main_modules)

        # Auxiliary Module
        self.auxiliary_module = nn.Sequential(nn.BatchNorm1d(aux_features), nn.Linear(aux_features, num_classes, bias=True))
